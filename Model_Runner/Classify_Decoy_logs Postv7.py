# -*- coding: utf-8 -*-
"""
Created on Mon Apr 17 22:53:04 2023
...
def plcFile(filename):
    ...
    if not imageNotFound:
        total_add = 1
        gray_prediction_Many = np.mean([predict_image(img, PLCmodel) for i, img in enumerate(gray_img_array) if i % 5 == 0], axis=0)
        gray_predicted_label_Many = np.argmax(gray_prediction_Many)
        colour_prediction_Many = np.mean([predict_image(img, PLCmodel) for i, img in enumerate(colour_img_array) if i % 5 == 0], axis=0)
        colour_predicted_label_Many = np.argmax(colour_prediction_Many)
        mean_pred_Many = np.mean([gray_prediction_Many, colour_prediction_Many], axis=0)
        correct = np.argmax(mean_pred_Many) == label
...
def updateGUI(window, colour_img, filename, snippet, label, mean_pred_Many, correcount, total, decoy):
    ...
    window['-SNIPPET-'].Widget.configure(highlightcolor=category_colors[num2cat(label, decoy)], highlightthickness=8)
    if window['-RADIO-'].get():
        time.sleep(0.2)
        window.refresh()
...
layout = [    [sg.Image(source='TitleImage4.png', key='-TITLE-', size=(1000, 100))],
    ...
    [sg.Text('Prediction: '), sg.Text(key='-PREDICTION-', font=('Helvetica', 12), size=(10, 1))],
    [sg.Text('Attack Type: '), sg.Text(key='-ATTACK-', font=('Helvetica', 12), size=(10, 1))],

# -*- coding: utf-8 -*-
"""
Created on Sun Apr 16 02:28:13 2023
...
layout = [
    ...
    [sg.Input(size=(25, 1), key="-FOLDER-"), sg.FolderBrowse(initial_folder=path), sg.Button("Classify Json")],
                ...
                if filename.endswith('.json'):
                    ...
                    if not type(colour_img) == str:
                        ...
                        if window['-RADIO-'].get():
                            time.sleep(2)
                            window.refresh()
                        window['-SNIPPET-'].Widget.configure(highlightcolor=category_colors[num2cat(label, True)], highlightthickness=4)
                        if window['-RADIO-'].get():
                            time.sleep(2)
                            window.refresh()
                        window['-ATTACK-'].update(num2cat(label, True), background_color=category_colors[num2cat(label, True)])
                        if window['-RADIO-'].get():
                            time.sleep(2)
                            window.refresh()
                        show_Image(window,colour_img,'-IMAGE-') 
                        if window['-RADIO-'].get():
                            time.sleep(2)
                            window.refresh()
                        window['-COLOR-'].update('', background_color=category_colors[num2cat(np.argmax(mean_pred_Many), True)])
                        window['-PREDICTION-'].update(num2cat(np.argmax(mean_pred_Many), True), background_color=category_colors[num2cat(np.argmax(mean_pred_Many), True)])
                        if window['-RADIO-'].get():
                            time.sleep(2)
                            window.refresh()
                        if correct:
